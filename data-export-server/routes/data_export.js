...
var exportDataMng = {
	...
	exportData: function (req, res) {
		...
		if(req.body.table == 'ip'){
			if(req.body.isGenre){
				_query += " FROM DevDiggit_Hist.Diggit_IP t, DevDiggit_Hist.mm_title_genres gt, DevDiggit_Hist.genres g WHERE t.Date BETWEEN '"+start+"' AND '"+end+"' AND t.TitleID = gt.title_id AND gt.genre_id IN "+genreQ; // LIMIT 10000
			} else {
